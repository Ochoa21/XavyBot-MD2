...
function sensor(text, customForbiddenWords = []) {
    const forbiddenWordsSet = new Set([...dataBadword, ...customForbiddenWords]);
    const regex = new RegExp([...forbiddenWordsSet].map(word => `\\b${word}\\b`).join("|"), "gi");
    return text.replace(regex, matched => "*".repeat(matched.length));
}
function cekBadword(text, customForbiddenWords = []) {
    const forbiddenWordsSet = new Set([...dataBadword, ...customForbiddenWords]);
    const regex = new RegExp([...forbiddenWordsSet].map(word => `\\b${word}\\b`).join("|"), "gi");
