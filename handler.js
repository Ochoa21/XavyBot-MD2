    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/Ochoa21/XavyBot-MD/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Ochoa;;;\nFN:Ochoa\nORG:Ochoa\nTITLE:\nitem1.TEL;waid=573146224366:+57 314 622 4366\nitem1.X-ABLabel:Ochoa\nX-WA-BIZ-DESCRIPTION:[❗] Solo pa vueltas importantes.\nX-WA-BIZ-NAME:Ochoa\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'Ochoa', contacts: [{ vcard }] } }, { quoted: callmsg });
